Ext.define('Appsetup.appsetup.web.com.controller.organization.contactmanagement.EmailCategoryMainController', {
     extend: 'Appsetup.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.EmailCategoryMainController',
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#EmailCategoryForm');
          this.updateFormUI(formPanel, 'Save', true); /** Adding data to tree and grid */
          if (data != null) {
               var grid = this.view.down('#EmailCategoryGrid');
               var tree = this.view.down('#EmailCategoryTree');
               if (method == 'PUT') {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#EmailCategoryForm');
          var grid = this.view.down('#EmailCategoryGrid');
          var tree = this.view.down('#EmailCategoryTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               ...
               var gridPanel = this.view.up().down('#EmailCategoryGrid');
               ...
               var formPanel = this.getView().up().down('#EmailCategoryForm');
               jsonData = {
                    'findKey': primaryKey
               }
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var data = this.findRecordById(this.view.restURL, jsonData);
          var treePanel = this.view.up().up().down('#EmailCategoryTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#EmailCategoryForm').down('#' + me.column.dataIndex + '').store;
     ...
     setGridConfig: function(data) {
          ...
          var gridStore = Ext.create('Ext.data.Store', {
               model: 'Appsetup.appsetup.shared.com.model.organization.contactmanagement.EmailCategoryModel',
               autoLoad: true,
