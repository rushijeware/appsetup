...
public class CommonUtilRepositoryImpl implements CommonUtilRepository {
	...
	public long getTotalPageCount(String jpqlQuery) throws Exception {
		long count = 0;
		EntityManager entitymanager = emfResource.getResource();
		Query query1 = entitymanager.createNamedQuery(jpqlQuery);
	...
	public List<CommonEntityInterface> findPageWiseData(String jpqlQuery, int pageSize, int pageNo) throws Exception {
		EntityManager entitymanager = emfResource.getResource();
		Query query = entitymanager.createNamedQuery(jpqlQuery);
		query.setFirstResult((pageNo - 1) * pageSize);
		query.setMaxResults(pageSize);
		List<CommonEntityInterface> list = query.getResultList();
		return list;
	}
