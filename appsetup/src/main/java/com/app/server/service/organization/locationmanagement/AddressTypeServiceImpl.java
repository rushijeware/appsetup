...
@SourceCodeAuthorClass(createdBy = "deepali", updatedBy = "deepali", versionNumber = "3", comments = "Service for AddressType Master table Entity", complexity = Complexity.LOW)
...
public class AddressTypeServiceImpl extends AddressTypeService {
    ...
    public HttpEntity<ResponseBean> findPageWiseData(@RequestParam("page") Integer page, @RequestParam("start") Integer start, @RequestParam("limit") Integer limit) throws Exception {
        ...
        ResponseBean responseBean = new ResponseBean(appAlarm);
        responseBean.add("message", String.format(appAlarm.getMessage(), "AddressType"));
        responseBean.add("data", addressTyperepo.findPageWiseData("AddressType.findAll", limit, page));
        responseBean.add("totalSize", addressTyperepo.getTotalPageCount("AddressType.findAll"));
