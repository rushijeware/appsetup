...
@SourceCodeAuthorClass(createdBy = "deepali", updatedBy = "deepali", versionNumber = "3", comments = "Service for Question Master table Entity", complexity = Complexity.LOW)
...
public class QuestionServiceImpl extends QuestionService {
    ...
    public HttpEntity<ResponseBean> findPageWiseData(@RequestParam("page") Integer page, @RequestParam("start") Integer start, @RequestParam("limit") Integer limit) throws Exception {
        ...
        ResponseBean responseBean = new ResponseBean(appAlarm);
        responseBean.add("message", String.format(appAlarm.getMessage(), "Question"));
        responseBean.add("data", questionrepo.findPageWiseData("Question.findAll", limit, page));
        responseBean.add("totalSize", questionrepo.getTotalPageCount("Question.findAll"));
