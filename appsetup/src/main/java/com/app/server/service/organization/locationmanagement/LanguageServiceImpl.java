...
@SourceCodeAuthorClass(createdBy = "deepali", updatedBy = "deepali", versionNumber = "3", comments = "Service for Language Master table Entity", complexity = Complexity.LOW)
...
public class LanguageServiceImpl extends LanguageService {
    ...
    public HttpEntity<ResponseBean> findPageWiseData(@RequestParam("page") Integer page, @RequestParam("start") Integer start, @RequestParam("limit") Integer limit) throws Exception {
        ...
        ResponseBean responseBean = new ResponseBean(appAlarm);
        responseBean.add("message", String.format(appAlarm.getMessage(), "Language"));
        responseBean.add("data", languagerepo.findPageWiseData("Language.findAll", limit, page));
        responseBean.add("totalSize", languagerepo.getTotalPageCount("Language.findAll"));
