...
@SourceCodeAuthorClass(createdBy = "deepali", updatedBy = "deepali", versionNumber = "3", comments = "Service for PasswordAlgo Master table Entity", complexity = Complexity.LOW)
...
public class PasswordAlgoServiceImpl extends PasswordAlgoService {
    ...
    public HttpEntity<ResponseBean> findPageWiseData(@RequestParam("page") Integer page, @RequestParam("start") Integer start, @RequestParam("limit") Integer limit) throws Exception {
        ...
        ResponseBean responseBean = new ResponseBean(appAlarm);
        responseBean.add("message", String.format(appAlarm.getMessage(), "PasswordAlgo"));
        responseBean.add("data", passwordAlgorepo.findPageWiseData("PasswordAlgo.findAll", limit, page));
        responseBean.add("totalSize", passwordAlgorepo.getTotalPageCount("PasswordAlgo.findAll"));
