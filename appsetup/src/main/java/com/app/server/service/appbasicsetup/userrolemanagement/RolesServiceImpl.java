...
@SourceCodeAuthorClass(createdBy = "deepali", updatedBy = "deepali", versionNumber = "3", comments = "Service for Roles Transaction table", complexity = Complexity.MEDIUM)
...
public class RolesServiceImpl extends RolesService {
    ...
    public HttpEntity<ResponseBean> findPageWiseData(@RequestParam("page") Integer page, @RequestParam("start") Integer start, @RequestParam("limit") Integer limit) throws Exception {
        ...
        ResponseBean responseBean = new ResponseBean(appAlarm);
        responseBean.add("message", String.format(appAlarm.getMessage(), "Roles"));
        responseBean.add("data", rolesrepo.findPageWiseData("Roles.findAll", limit, page));
        responseBean.add("totalSize", rolesrepo.getTotalPageCount("Roles.findAll"));
