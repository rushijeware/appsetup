...
@SourceCodeAuthorClass(createdBy = "deepali", updatedBy = "deepali", versionNumber = "3", comments = "Service for ContactType Master table Entity", complexity = Complexity.LOW)
...
public class ContactTypeServiceImpl extends ContactTypeService {
    ...
    public HttpEntity<ResponseBean> findPageWiseData(@RequestParam("page") Integer page, @RequestParam("start") Integer start, @RequestParam("limit") Integer limit) throws Exception {
        ...
        ResponseBean responseBean = new ResponseBean(appAlarm);
        responseBean.add("message", String.format(appAlarm.getMessage(), "ContactType"));
        responseBean.add("data", contactTyperepo.findPageWiseData("ContactType.findAll", limit, page));
        responseBean.add("totalSize", contactTyperepo.getTotalPageCount("ContactType.findAll"));
