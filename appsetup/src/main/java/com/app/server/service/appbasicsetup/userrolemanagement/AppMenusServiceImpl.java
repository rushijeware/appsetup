...
@SourceCodeAuthorClass(createdBy = "deepali", updatedBy = "deepali", versionNumber = "3", comments = "Service for AppMenus Master table Entity", complexity = Complexity.LOW)
...
public class AppMenusServiceImpl extends AppMenusService {
    ...
    public HttpEntity<ResponseBean> findPageWiseData(@RequestParam("page") Integer page, @RequestParam("start") Integer start, @RequestParam("limit") Integer limit) throws Exception {
        ...
        ResponseBean responseBean = new ResponseBean(appAlarm);
        responseBean.add("message", String.format(appAlarm.getMessage(), "AppMenus"));
        responseBean.add("data", appMenusrepo.findPageWiseData("AppMenus.findAll", limit, page));
        responseBean.add("totalSize", appMenusrepo.getTotalPageCount("AppMenus.findAll"));
