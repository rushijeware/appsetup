...
@SourceCodeAuthorClass(createdBy = "deepali", updatedBy = "deepali", versionNumber = "3", comments = "Service for Login Transaction table", complexity = Complexity.MEDIUM)
...
public class LoginServiceImpl extends LoginService {
    ...
    public HttpEntity<ResponseBean> findPageWiseData(@RequestParam("page") Integer page, @RequestParam("start") Integer start, @RequestParam("limit") Integer limit) throws Exception {
        ...
        ResponseBean responseBean = new ResponseBean(appAlarm);
        responseBean.add("message", String.format(appAlarm.getMessage(), "Login"));
        responseBean.add("data", loginrepo.findPageWiseData("Login.findAll", limit, page));
        responseBean.add("totalSize", loginrepo.getTotalPageCount("Login.findAll"));
