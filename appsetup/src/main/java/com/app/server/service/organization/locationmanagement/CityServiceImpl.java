...
@SourceCodeAuthorClass(createdBy = "deepali", updatedBy = "deepali", versionNumber = "3", comments = "Service for City Master table Entity", complexity = Complexity.LOW)
@RequestMapping("/City")
public class CityServiceImpl extends CityService {
    ...
    public HttpEntity<ResponseBean> findPageWiseData(@RequestParam("page") Integer page, @RequestParam("start") Integer start, @RequestParam("limit") Integer limit) throws Exception {
        ...
        ResponseBean responseBean = new ResponseBean(appAlarm);
        responseBean.add("message", String.format(appAlarm.getMessage(), "City"));
        responseBean.add("data", cityrepo.findPageWiseData("City.findAll", limit, page));
        responseBean.add("totalSize", cityrepo.getTotalPageCount("City.findAll"));
