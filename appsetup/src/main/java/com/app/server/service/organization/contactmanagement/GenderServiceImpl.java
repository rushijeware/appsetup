...
@SourceCodeAuthorClass(createdBy = "deepali", updatedBy = "deepali", versionNumber = "3", comments = "Service for Gender Master table Entity", complexity = Complexity.LOW)
...
public class GenderServiceImpl extends GenderService {
    ...
    public HttpEntity<ResponseBean> findPageWiseData(@RequestParam("page") Integer page, @RequestParam("start") Integer start, @RequestParam("limit") Integer limit) throws Exception {
        ...
        ResponseBean responseBean = new ResponseBean(appAlarm);
        responseBean.add("message", String.format(appAlarm.getMessage(), "Gender"));
        responseBean.add("data", genderrepo.findPageWiseData("Gender.findAll", limit, page));
        responseBean.add("totalSize", genderrepo.getTotalPageCount("Gender.findAll"));
