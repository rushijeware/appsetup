...
@SourceCodeAuthorClass(createdBy = "deepali", updatedBy = "deepali", versionNumber = "3", comments = "Service for Address Transaction table", complexity = Complexity.MEDIUM)
...
public class AddressServiceImpl extends AddressService {
    ...
    public HttpEntity<ResponseBean> findPageWiseData(@RequestParam("page") Integer page, @RequestParam("start") Integer start, @RequestParam("limit") Integer limit) throws Exception {
        ...
        ResponseBean responseBean = new ResponseBean(appAlarm);
        responseBean.add("message", String.format(appAlarm.getMessage(), "Address"));
        responseBean.add("data", addressrepo.findPageWiseData("Address.findAll", limit, page));
        responseBean.add("totalSize", addressrepo.getTotalPageCount("Address.findAll"));
