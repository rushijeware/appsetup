...
@SourceCodeAuthorClass(createdBy = "deepali", updatedBy = "deepali", versionNumber = "3", comments = "Service for CoreContacts Transaction table", complexity = Complexity.MEDIUM)
...
public class CoreContactsServiceImpl extends CoreContactsService {
    ...
    public HttpEntity<ResponseBean> findPageWiseData(@RequestParam("page") Integer page, @RequestParam("start") Integer start, @RequestParam("limit") Integer limit) throws Exception {
        ...
        ResponseBean responseBean = new ResponseBean(appAlarm);
        responseBean.add("message", String.format(appAlarm.getMessage(), "CoreContacts"));
        responseBean.add("data", coreContactsrepo.findPageWiseData("CoreContacts.findAll", limit, page));
        responseBean.add("totalSize", coreContactsrepo.getTotalPageCount("CoreContacts.findAll"));
