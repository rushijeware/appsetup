...
@SourceCodeAuthorClass(createdBy = "deepali", updatedBy = "deepali", versionNumber = "3", comments = "Service for PasswordPolicy Master table Entity", complexity = Complexity.LOW)
...
public class PasswordPolicyServiceImpl extends PasswordPolicyService {
    ...
    public HttpEntity<ResponseBean> findPageWiseData(@RequestParam("page") Integer page, @RequestParam("start") Integer start, @RequestParam("limit") Integer limit) throws Exception {
        ...
        ResponseBean responseBean = new ResponseBean(appAlarm);
        responseBean.add("message", String.format(appAlarm.getMessage(), "PasswordPolicy"));
        responseBean.add("data", passwordPolicyrepo.findPageWiseData("PasswordPolicy.findAll", limit, page));
        responseBean.add("totalSize", passwordPolicyrepo.getTotalPageCount("PasswordPolicy.findAll"));
