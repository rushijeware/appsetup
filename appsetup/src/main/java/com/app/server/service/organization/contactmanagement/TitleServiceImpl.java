...
@SourceCodeAuthorClass(createdBy = "deepali", updatedBy = "deepali", versionNumber = "3", comments = "Service for Title Master table Entity", complexity = Complexity.LOW)
...
public class TitleServiceImpl extends TitleService {
    ...
    public HttpEntity<ResponseBean> findPageWiseData(@RequestParam("page") Integer page, @RequestParam("start") Integer start, @RequestParam("limit") Integer limit) throws Exception {
        ...
        ResponseBean responseBean = new ResponseBean(appAlarm);
        responseBean.add("message", String.format(appAlarm.getMessage(), "Title"));
        responseBean.add("data", titlerepo.findPageWiseData("Title.findAll", limit, page));
        responseBean.add("totalSize", titlerepo.getTotalPageCount("Title.findAll"));
