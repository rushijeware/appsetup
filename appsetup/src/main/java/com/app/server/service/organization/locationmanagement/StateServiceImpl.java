...
@SourceCodeAuthorClass(createdBy = "deepali", updatedBy = "deepali", versionNumber = "3", comments = "Service for State Master table Entity", complexity = Complexity.LOW)
...
public class StateServiceImpl extends StateService {
    ...
    public HttpEntity<ResponseBean> findPageWiseData(@RequestParam("page") Integer page, @RequestParam("start") Integer start, @RequestParam("limit") Integer limit) throws Exception {
        ...
        ResponseBean responseBean = new ResponseBean(appAlarm);
        responseBean.add("message", String.format(appAlarm.getMessage(), "State"));
        responseBean.add("data", staterepo.findPageWiseData("State.findAll", limit, page));
        responseBean.add("totalSize", staterepo.getTotalPageCount("State.findAll"));
