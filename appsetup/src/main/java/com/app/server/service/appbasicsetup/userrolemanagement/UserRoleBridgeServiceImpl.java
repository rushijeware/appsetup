...
@SourceCodeAuthorClass(createdBy = "deepali", updatedBy = "deepali", versionNumber = "3", comments = "Service for UserRoleBridge Transaction table", complexity = Complexity.MEDIUM)
...
public class UserRoleBridgeServiceImpl extends UserRoleBridgeService {
    ...
    public HttpEntity<ResponseBean> findPageWiseData(@RequestParam("page") Integer page, @RequestParam("start") Integer start, @RequestParam("limit") Integer limit) throws Exception {
        ...
        ResponseBean responseBean = new ResponseBean(appAlarm);
        responseBean.add("message", String.format(appAlarm.getMessage(), "UserRoleBridge"));
        responseBean.add("data", userRoleBridgerepo.findPageWiseData("UserRoleBridge.findAll", limit, page));
        responseBean.add("totalSize", userRoleBridgerepo.getTotalPageCount("UserRoleBridge.findAll"));
